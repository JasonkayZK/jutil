...
public class DownloadUtils {
     ...
     * 默认下载目录
     */
    private static String storageFolder = System.getProperty("user.home") + File.separator + "util";
    ...
    public static boolean download(String storageFolder, String downloadURL) {
        DownloadUtils.storageFolder = storageFolder;
    ...
    public static boolean download(String downloadURL) {
        ...
        try {
            String fileName = checkPath(storageFolder + File.separator + Formatter.getFileName(downloadURL));
            ...
            if (conn.getResponseCode() != NetUtils.RESPONSE_OK || conn.getContentLength() <= 0) {
                return false;
            }
